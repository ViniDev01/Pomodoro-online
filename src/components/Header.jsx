...
export default function Header({ setAtual, atual, setVerLogin, verLogin }) {


    return (
        <header className={atual ? "theme-light" : ""}>
            <h1 className={`text-2xl ${atual ? "text-black" : "text-white"}`}>Timer Pomodoro</h1>
            ...
            <div className="flex items-center gap-6">
                <label className="switch">
                    ...
                    <input type="checkbox" className="input" onChange={(e) => setAtual(e.target.checked)}/>
                    <span className="slider"></span>
                </label>

                <UserRound className="p-2 w-10 h-10 bg-white cursor-pointer rounded-full" onClick={() => setVerLogin(true)} />
            </div>
